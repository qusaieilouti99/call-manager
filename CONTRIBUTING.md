...
If you want to use Android Studio or XCode to edit the native code, you can open the `example/android` or `example/ios` directories respectively in those editors. To edit the Objective-C or Swift files, open `example/ios/CallManagerExample.xcworkspace` in XCode and find the source files at `Pods > Development Pods > @qusaieilouti99/call-manager`.

To edit the Java or Kotlin files, open `example/android` in Android studio and find the source files at `qusaieilouti99-call-manager` under `Android`.
...
```sh
Running "CallManagerExample" with {"fabric":true,"initialProps":{"concurrentRoot":true},"rootTag":1}
```
